"""
Run backtest. Usage: python backtest.py
Imports strategy from strategy.py, runs on validation data, prints metrics.
This file is fixed — do not modify.

Windows-compatible: uses no SIGALRM or multiprocessing.
The time budget is enforced inside run_backtest() in prepare.py via time.time() checks.
"""
